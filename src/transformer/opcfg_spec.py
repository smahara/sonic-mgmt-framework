...
class OpcfgSpec(object):
    ...
    def __init__(self, path, spec):
        ...
        opcfg_log.log_dbg_msg(opcfg_spec_filenm + ":%s"%sys._getframe().f_lineno, "PATH : %s"%path)
    ...
    def _process_table_info(self, yng_rds_map_ky, table_name, parent_table_orddict, yobject_key_inht = None): 
        ...
        if parent_table_orddict is None:
            ...
            key_inht =  len(parent_table_orddict) # this is default
        if yobject_key_inht is not None:
            key_inht = yobject_key_inht
            # @todo enhance to specify how many parent levels should be inherited
        else:
            ky_idx = key_inht
            sum_keys = []
            key_list = parent_table_orddict.keys()
            for idx in key_list:
                sum_keys = self._rds_tbl_data[idx]['table_keys'] + sum_keys
            sum_keys = sum_keys + self._rds_tbl_data[table_name]['table_keys']
