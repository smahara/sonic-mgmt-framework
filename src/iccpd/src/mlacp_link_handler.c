...
void mlacp_peerlink_up_handler(struct CSM* csm)
{
    ...
    RB_FOREACH (mac_msg, mac_rb_tree, &MLACP(csm).mac_rb)
    {
        ...
        ICCPD_LOG_DEBUG(__FUNCTION__, "Peer link up, add MAC to ASIC for peer-link: %s, "
                "MAC %s vlan-id %d", mac_msg->ifname, mac_addr_to_str(mac_msg->mac_addr), mac_msg->vid);
...
void mlacp_peerlink_down_handler(struct CSM* csm)
{
    ...
    RB_FOREACH (mac_msg, mac_rb_tree, &MLACP(csm).mac_rb)
    {
        ...
        ICCPD_LOG_DEBUG(__FUNCTION__, "Peer link down, del MAC for peer-link: %s,"
            " MAC %s vlan-id %d", mac_msg->ifname, mac_addr_to_str(mac_msg->mac_addr), mac_msg->vid);
...
void do_mac_update_from_syncd(uint8_t mac_addr[ETHER_ADDR_LEN], uint16_t vid, char *ifname, uint8_t fdb_type, uint8_t op_type)
{
    ...
    {
        mac_exist = 1;
        ICCPD_LOG_DEBUG(__FUNCTION__, " RB_FIND success for the MAC entry : %s, "
    ...
    if (op_type == MAC_SYNC_ADD)
    {
        ...
        if (mac_exist)
        {
                ...
                || strcmp(mac_info->origin_ifname, mac_msg->ifname) != 0)
            {
                ...
                {
                    ICCPD_LOG_DEBUG(__FUNCTION__, "MAC move Update MAC remote to local %s, vlan %d"
            ...
            else
            {
                ...
                if (!(mac_msg->age_flag & MAC_AGE_PEER))
                {
                    ICCPD_LOG_DEBUG(__FUNCTION__, "Update MAC remote to local %s, vlan %d"
                            " ifname %s, del entry from MCLAG_FDB_TABLE",
                            mac_addr_to_str(mac_msg->mac_addr), mac_msg->vid, mac_msg->ifname);
                    del_mac_from_chip(mac_msg);
                }
...
void do_update_from_l2mc(uint8_t saddr[16], uint16_t vid, uint8_t gaddr[16], char *ifname, uint8_t l2mc_type, uint8_t op_type, int leave)
{
    ...
    if(l2mc_info)
    {
        ...
        ICCPD_LOG_DEBUG(__FUNCTION__, " RB_FIND success for the L2MC entry : %s, %s, "
            " vid: %d , ifname %s", saddr, gaddr,
            l2mc_info->vid, l2mc_info->ifname );
    }
