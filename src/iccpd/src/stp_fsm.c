...
static void stp_process_peer_common_info(struct CSM* csm, struct Msg* msg)
{
    ...
    info->config_pending = data->config_pending;
    ...
    info->kernel_state = data->kernel_state;
    info->path_cost = ntohs(data->path_cost);
    info->desig_cost = ntohl(data->desig_cost);
    info->message_age_timer = ntohs(data->message_age_timer);
    info->forward_delay = data->forward_delay;
    info->hold_time = data->hold_time;
    info->root_protect_timer = ntohs(data->root_protect_timer);
    ...
    ICCPD_LOG_DEBUG(__FUNCTION__, "stp_common_info from peer (2)-- state:%d tc_ack:%d config_pending:%d change_detection_enabled:%d self_loop:%d auto_config:%d oper_edge:%d kernel_state:%d path_cost:%d desig_cost:%d message_age_timer:%d forward_delay:%d hold_time:%d root_protect_timer:%d ", info->state, info->tc_ack, info->config_pending, info->change_detection_enabled, info->self_loop, info->auto_config, info->oper_edge, info->kernel_state, info->path_cost, info->desig_cost, info->message_age_timer, info->forward_delay, info->hold_time, info->root_protect_timer);
    ICCPD_LOG_DEBUG(__FUNCTION__, "stp_common_info from peer (3)-- master_node_req_resp %d rpvst_req_prop_ack_flag %d message_type: %u", info->master_node_req_resp_field, info->rpvst_req_proposal_ack_flag, info->message_type);
