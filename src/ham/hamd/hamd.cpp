...
hamd_c::hamd_c(const config_c & config_r, DBus::Connection & conn_r) :
    ...
    sac_uid_min_m(config_r.sac_uid_min_m),
    sac_uid_max_m(config_r.sac_uid_max_m),
    sac_uid_range_m(1 + (config_r.sac_uid_max_m - config_r.sac_uid_min_m)),
    tron_m(config_r.verbose_m),
    poll_timer_m((double)config_r.poll_period_sec_m, hamd_c::on_poll_timeout, this)
{
    poll_timer_m.start();
}
...
void hamd_c::rm_unconfirmed_users() const
{
    ...
    if (f)
    {
        ...
        {
            const char * pid_p;
            if ((ent->pw_uid >= (uid_t)sac_uid_min_m) && (ent->pw_uid <= (uid_t)sac_uid_max_m) &&
...
bool hamd_c::add_unconfirmed_user(const std::string& username, const uint32_t& pid)
{
    ...
    for (n_tries = 0; n_tries < 100; n_tries++) /* Give up retrying eventually */
    {
        ...
        candidate = uid_fit_into_range(siphash24(name.c_str(), name.length(), hash_key));
        ...
        if (!::getpwuid(candidate)) /* make sure not already allocated */
        {
