...
func clear_arp_all(fam_switch string, force bool, vrf string) string {
    var err error
...
func clear_arp_ip(ip string, fam_switch string, force bool, vrf string) string {
    var cmd *exec.Cmd
...
var rpc_clear_neighbors RpcCallpoint = func(body []byte, dbs [db.MaxDB]*db.DB) ([]byte, error) {
    ...
    if len(intf) > 0 {
        ...
        status = clear_arp_ip(ip, fam_switch, force, vrf)
    } else if len(intf) <= 0 && len(ip) <= 0 {
        status = clear_arp_all(fam_switch, force, vrf)
    }
