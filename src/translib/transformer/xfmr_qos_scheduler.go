...
var YangToDb_qos_scheduler_priority_xfmr FieldXfmrYangToDb = func(inParams XfmrParams) (map[string]string, error) {
    var db_qos_scheduler_priority string
    ...
    if qos_scheduler_priority == ocbinds.OpenconfigQos_Qos_SchedulerPolicies_SchedulerPolicy_Schedulers_Scheduler_Config_Priority_STRICT {
        db_qos_scheduler_priority = "PRIORITY"
    } else {
        db_qos_scheduler_priority = "WRR"
    }

    log.Info( "YangToDb_qos_scheduler_priority_xfmr: type: ", db_qos_scheduler_priority)
    res_map :=  make(map[string]string)
    res_map["type"] = db_qos_scheduler_priority 
    return res_map, nil
}

var DbToYang_qos_scheduler_priority_xfmr KeyXfmrDbToYang = func(inParams XfmrParams) (map[string]interface{}, error) {
    var err error
    rmap := make(map[string]interface{})
    data := (*inParams.dbDataMap)[inParams.curDb]
    db_qos_scheduler_priority, ok := data["SCHEDULER"][inParams.key].Field["type"]
    if ok {
        log.Info("DbToYang_qos_scheduler_priority_xfmr: db_qos_scheduler_priority: ", db_qos_scheduler_priority)
        if db_qos_scheduler_priority == "PRIORITY" {
            rmap["priority"] = ocbinds.OpenconfigQos_Qos_SchedulerPolicies_SchedulerPolicy_Schedulers_Scheduler_Config_Priority_STRICT
        } else {
            rmap["priority"] = ocbinds.OpenconfigQos_Qos_SchedulerPolicies_SchedulerPolicy_Schedulers_Scheduler_Config_Priority_UNSET
        }
    }
    return rmap, err
}
