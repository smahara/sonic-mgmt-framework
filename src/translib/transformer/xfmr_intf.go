...
var rpc_renew_dhcp_lease RpcCallpoint = func(body []byte, dbs [db.MaxDB]*db.DB) ([]byte, error) {
    ...
    if ierr != nil || intfType != IntfTypeMgmt {
        ...
        result.Output.Status_detail = fmt.Sprintf("Error: Not supported for interface " + ifName)
        return json.Marshal(&result)
    }
    ...
    if query_result.Err != nil {
        ...
        result.Output.Status_detail = fmt.Sprintf("ERROR:Invalid renew DHCP lease request " + ifName)
    } else {
        result.Output.Status = 0
        result.Output.Status_detail = "Success: Renew DHCP lease"
    }
    return json.Marshal(&result)
}
