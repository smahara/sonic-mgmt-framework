...
func init () {
    ...
    XlateFuncBind("YangToDb_intf_subintf_ip_xfmr", YangToDb_intf_subintf_ip_xfmr)
    XlateFuncBind("DbToYang_intf_subintf_ip_xfmr", DbToYang_intf_subintf_ip_xfmr)
    ...
    //XlateFuncBind("YangToDb_intf_counters_key", YangToDb_intf_counters_key)
    //XlateFuncBind("DbToYang_intf_counters_key", DbToYang_intf_counters_key)
...
/*
func intf_intf_tbl_key_gen (intfName string, ip string, prefixLen int, keySep string) {
    return intfName + keySep + ip + "/" + strconv.Itoa(prefixLen)
}

func intf_intf_tbl_key_split (key db.Key) (intfName string, ip string, prefixLen) {

}
*/
var YangToDb_intf_subintf_ip_xfmr SubTreeXfmrYangToDb = func(inParams XfmrParams) (map[string]map[string]db.Value, error) {
    var err error
    subIntfmap := make(map[string]map[string]db.Value)
    //ipTblMap := make(map[string]db.Value)
    ...
    intfIPTs := &db.TableSpec{Name: tblName}
    //allIpkeys,_ := doGetAllIpKeys(inParams.d,  &db.TableSpec{Name: tblName})
    
    if inParams.oper == DELETE {
    }
    var allIpkeys []db.Key
    for _, intTbl := range IntfTypeTblMap {
        ipKeys, _ := doGetAllIpKeys(inParams.d,  &db.TableSpec{Name: intTbl.cfgDb.intfTN})
        allIpkeys = append(allIpkeys, ipKeys...)

    }
    var ifIPTableMap map[string]map[string]dbEntry
        ...
        for ip, _ := range subIntfObj.Ipv4.Addresses.Address {
            ...
            if addr.Config != nil {
                ...
                if inParams.oper == DELETE {
                    ...
                    ifIPTableMap, err = translateIpv4(inParams.dbs[db.ConfigDB], allIpkeys, ifName, *addr.Config.Ip, int(*addr.Config.PrefixLength))
                    if err != nil {
                        return subIntfmap, err
                    }
                }
                //ipTblKey := intf_intf_tbl_key_gen(ifName, *addr.Config.Ip, int(*addr.Config.PrefixLength), intTbl.cfgDb.keySep)
    ...
    for key, entry1 := range ifIPTableMap {
        for ip, entry := range entry1 {
            if entry.op == opCreate {
                log.Info("Creating entry for ", key, ":", ip)
                err = inParams.d.CreateEntry(intfIPTs, db.Key{Comp: []string{key, ip}}, entry.entry)
            } else if entry.op == opDelete {
                log.Info("Deleting entry for ", key, ":", ip)
                err = inParams.d.DeleteEntry(intfIPTs, db.Key{Comp: []string{key, ip}})
            }
        }
    }
    return subIntfmap, err
...
func convertIpMapToOC (intfIpMap map[string]dbEntry, ifInfo *ocbinds.OpenconfigInterfaces_Interfaces_Interface, isState bool) error {
...
func getIntfIpByName(dbCl *db.DB, tblName string, ifName string, ipv4 bool, ipv6 bool) (map[string]dbEntry, error) {
    var err error
    intfIpMap := make(map[string]dbEntry)
    all := true
    ...
    for _, key := range keys {
        ...
        intfIpMap[key.Get(1)]= dbEntry{entry: ipInfo}
    }
...
func handleIntfIPGetByTargetURI (inParams XfmrParams, targetUriPath string, ifName string, intfObj *ocbinds.OpenconfigInterfaces_Interfaces_Interface) error {
    ...
    if strings.HasPrefix(targetUriPath, "/openconfig-interfaces:interfaces/interface/subinterfaces/subinterface/ipv4/addresses/address/config") ||
       strings.HasPrefix(targetUriPath, "/openconfig-interfaces:interfaces/interface/subinterfaces/subinterface/openconfig-if-ip:ipv4/addresses/address/config") {
           ipMap, err = getIntfIpByName(inParams.dbs[db.ConfigDB], intTbl.cfgDb.intfTN, ifName, true, false)
        ...
        strings.HasPrefix(targetUriPath, "/openconfig-interfaces:interfaces/interface/subinterfaces/subinterface/ipv6/addresses/address/config") {
           ipMap, err = getIntfIpByName(inParams.dbs[db.ConfigDB], intTbl.cfgDb.intfTN, ifName, false, true)
           log.Info("handleIntfIPGetByTargetURI : ipv6 config ipMap - : ", ipMap)
           convertIpMapToOC(ipMap, intfObj, false)
    } else if strings.HasPrefix(targetUriPath, "/openconfig-interfaces:interfaces/interface/subinterfaces/subinterface/ipv4/addresses/address/state") ||
         strings.HasPrefix(targetUriPath, "/openconfig-interfaces:interfaces/interface/subinterfaces/subinterface/openconfig-if-ip:ipv4/addresses/address/state") {
           ipMap, err = getIntfIpByName(inParams.dbs[db.ApplDB], intTbl.appDb.intfTN, ifName, true, false)
         ...
         strings.HasPrefix(targetUriPath, "/openconfig-interfaces:interfaces/interface/subinterfaces/subinterface/ipv6/addresses/address/state") {
           ipMap, err = getIntfIpByName(inParams.dbs[db.ApplDB], intTbl.appDb.intfTN, ifName, false, true)
           log.Info("handleIntfIPGetByTargetURI : ipv6 state ipMap - : ", ipMap)
           convertIpMapToOC(ipMap, intfObj, true)
    } else if strings.HasPrefix(targetUriPath, "/openconfig-interfaces:interfaces/interface/subinterfaces/subinterface") {
        ipMap, err = getIntfIpByName(inParams.dbs[db.ConfigDB], intTbl.cfgDb.intfTN, ifName, true, true)
           log.Info("handleIntfIPGetByTargetURI : ipv4 and ipv6 config ipMap - : ", ipMap)
        convertIpMapToOC(ipMap, intfObj, false)
        ipMap, err = getIntfIpByName(inParams.dbs[db.ApplDB], intTbl.appDb.intfTN, ifName, true, true)
...
var DbToYang_intf_subintf_ip_xfmr SubTreeXfmrDbToYang = func (inParams XfmrParams) (error) {
    var err error
    intfsObj := getIntfsRoot(inParams.ygRoot)
    pathInfo := NewPathInfo(inParams.uri)
...
func translateIpv4(d *db.DB, allIpKeys []db.Key, intf string, ip string, prefix int) (map[string]map[string]dbEntry, error) {
    var err error
    var ifsKey db.Key
    var ifIPTableMap map[string]map[string]dbEntry
    ...
    for _, key := range allIpKeys {
        ...
        if ipNetA.Contains(ipB) || ipNetB.Contains(ipA) {
            ...
            if intf != key.Get(0) {
                ...
                return ifIPTableMap, err
            } else {
                var entry dbEntry
                entry.op = opDelete
                log.Info("Entry ", key.Get(1), " on ", intf, " needs to be deleted")
                if ifIPTableMap[intf] == nil {
                    ifIPTableMap[intf] = make(map[string]dbEntry)
                }
                ifIPTableMap[intf][key.Get(1)] = entry
            }
        }
    }
    {
        ...
        ifIPTableMap[intf][ipPref] = entry
    }
    return ifIPTableMap, err
}
...
var populatePortCounters PopulateIntfCounters = func (inParams XfmrParams, counter *ocbinds.OpenconfigInterfaces_Interfaces_Interface_State_Counters) (error) {
    ...
    log.Infof("----------------------------------------------Counters called for interface %s\n", intfName)
...
var DbToYang_intf_get_counters_xfmr SubTreeXfmrDbToYang = func(inParams XfmrParams) error {
    ...
    targetUriPath, err := getYangPathFromUri(inParams.uri)
    log.Info("targetUriPath is ", targetUriPath)
    log.Infof("------------------Starting func---------------Counter request for %s\n", intfName)
