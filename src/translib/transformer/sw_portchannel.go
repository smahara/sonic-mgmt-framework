...
var DbToYang_lag_min_links_xfmr FieldXfmrDbtoYang = func(inParams XfmrParams) (map[string]interface{}, error) {
    ...
    if ierr != nil || intfType != IntfTypePortChannel  {
        log.Info("DbToYang_lag_min_links_xfmr Error: Invalid interface: ", inParams.key)
        return result, err
    }
    ...
    if ok {
        ...
        if log.V(3) {
            log.Info("min_links field not found in DB")
        }
    }
...
var DbToYang_lag_fallback_xfmr FieldXfmrDbtoYang = func(inParams XfmrParams) (map[string]interface{}, error) {
    ...
    intfType, _, ierr := getIntfTypeByName(inParams.key)
    if ierr != nil || intfType != IntfTypePortChannel {
        log.Info("DbToYang_lag_fallback_xfmr Error: Invalid interface: ", inParams.key)
        return result, err
    }
    ...
    if ok {
        ...
        if log.V(3) {
            log.Info("fallback field not found in DB")
        }
    }
    return result, err
}
