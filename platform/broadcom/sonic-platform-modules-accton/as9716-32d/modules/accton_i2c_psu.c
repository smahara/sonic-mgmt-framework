...
#define PMBUS_REGISTER_FAN_DIRECTION            0xC3
...
static int accton_i2c_psu_write_word(struct i2c_client *client, u8 reg,
                                     u16 value);
static struct accton_i2c_psu_data *accton_i2c_psu_update_device(
  struct device *dev);
 ...
 */
static SENSOR_DEVICE_ATTR(psu_v_in,        S_IRUGO, show_linear,      NULL,
                          PSU_V_IN);
static SENSOR_DEVICE_ATTR(psu_v_out,       S_IRUGO, show_vout,        NULL,
                          PSU_V_OUT);
static SENSOR_DEVICE_ATTR(psu_i_in,        S_IRUGO, show_linear,      NULL,
                          PSU_I_IN);
static SENSOR_DEVICE_ATTR(psu_i_out,       S_IRUGO, show_linear,      NULL,
                          PSU_I_OUT);
static SENSOR_DEVICE_ATTR(psu_p_in,        S_IRUGO, show_linear,      NULL,
                          PSU_P_IN);
static SENSOR_DEVICE_ATTR(psu_p_out,       S_IRUGO, show_linear,      NULL,
                          PSU_P_OUT);
...
static SENSOR_DEVICE_ATTR(curr2_input, S_IRUGO, show_linear,    NULL,
                          PSU_I_OUT);
static SENSOR_DEVICE_ATTR(power2_input, S_IRUGO, show_linear,    NULL,
                          PSU_P_OUT_UV);
                                  ...
                                  const char *buf, size_t count)
{
  ...
  accton_i2c_psu_write_word(client, PMBUS_REGISTER_FAN_COMMAND_1 + nr,
                         ...
                         char *buf)
{
  ...
  return (exponent > 0) ? sprintf(buf, "%d\n",
         ...
         sprintf(buf, "%d\n", (mantissa * PMBUS_LITERAL_DATA_MULTIPLIER) /
                 (1 << -exponent));
}
...
static int accton_i2c_psu_read_block(struct i2c_client *client, u8 command,
                                     u8 *data,
                                     int data_len)
{
...
static int accton_i2c_psu_read_block_data(struct i2c_client *client, u8 command,
  ...
  struct device *dev)
{
      ...
      || !data->valid)
  {
    ...
    /* Read mfr_id */
    status = accton_i2c_psu_read_block_data(client, PMBUS_REGISTER_MFR_ID,
                                            data->mfr_id,
    ...
    /* Read mfr_model */
    status = accton_i2c_psu_read_block_data(client, PMBUS_REGISTER_MFR_MODEL,
                                            data->mfr_model,
    ...
    /* Read mfr_revsion */
    status = accton_i2c_psu_read_block_data(client, PMBUS_REGISTER_MFR_REVISION,
                                            data->mfr_revsion,
    ...
    /* Read mfr_serial */
    status = accton_i2c_psu_read_block_data(client, PMBUS_REGISTER_MFR_SERIAL,
                                            data->mfr_serial,
    ...
    /* Read fan_direction */
    status = accton_i2c_psu_read_block_data(client, PMBUS_REGISTER_FAN_DIRECTION,
                                            data->fan_dir, ARRAY_SIZE(data->fan_dir) - 1);

    if (status < 0)
    {
      dev_dbg(&client->dev, "reg %d, err %d\n", PMBUS_REGISTER_FAN_DIRECTION, status);
      goto exit;
    }
