...
#define MODPRS_TRIG     2
#define RXLOS_TRIG      4


/* SFP PORT STATUS
 * [7:4] RESERVED
 * [3:0] TX_FAULT / MODPRS / RXLOS
 */
#define SFP_TXFAULT     0x0242
#define SFP_MODPRS      0x0243
...
static ssize_t sfp_modprs_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    unsigned char data;
    mutex_lock(&cpld_data->cpld_lock);
    data = inb(SFP_MODPRS);
 ...
 * @return     Current trigger mode.
 */
static ssize_t modprs_trig_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    unsigned char mode;
    char *mode_str[5] = {"falling", "rising", "both", "low"};

    mutex_lock(&cpld_data->cpld_lock);
    mode = inb(TRIG_MODE) >> MODPRS_TRIG;
    mode = mode & 0x3;
    mutex_unlock(&cpld_data->cpld_lock);
    return sprintf(buf, "%s\n", mode_str[mode]);
}
 ...
 */
static ssize_t modprs_trig_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    ssize_t status;
    unsigned char data, trig_mode;
    ...
    mutex_lock(&cpld_data->cpld_lock);
    data = inb(TRIG_MODE);
    data = data & ~(0x03 << MODPRS_TRIG);
    data = data | trig_mode << MODPRS_TRIG;
    outb(data, TRIG_MODE);
    mutex_unlock(&cpld_data->cpld_lock);
    status = size;
    return status;
...
static ssize_t modprs_int_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    unsigned char data;
    mutex_lock(&cpld_data->cpld_lock);
    data = inb(MODPRS_INT);
...
static ssize_t modprs_mask_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    unsigned char data;
    mutex_lock(&cpld_data->cpld_lock);
    data = inb(MODPRS_MSK);
...
static ssize_t modprs_mask_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    long value;
    ssize_t status;
...
static const struct attribute_group *lpc_groups[] = {
    &cpld_group,
    &sfp_group,
    NULL
};

static struct resource dell_s3000_lpc_cpld_resources[] = {
    {
        .start  = 0x0200,
        .end    = 0x0255,
        .flags  = IORESOURCE_IO,
...
static void dell_s3000_lpc_cpld_dev_release( struct device * dev)
{
        return;
}

static struct platform_device dell_s3000_lpc_cpld_dev = {
        .name           = "LPC",
        .id             = -1,
        .num_resources  = ARRAY_SIZE(dell_s3000_lpc_cpld_resources),
        .resource       = dell_s3000_lpc_cpld_resources,
        .dev = {
                        .release = dell_s3000_lpc_cpld_dev_release,
        }
};


static int dell_s3000_lpc_cpld_probe(struct platform_device *pdev)
{
    struct resource *res;
    ...
    hwmon_dev = devm_hwmon_device_register_with_groups(&pdev->dev,
                        "S3000_LPC", NULL, lpc_groups);
    outb(0xFF, SPR_RESET);

    return PTR_ERR_OR_ZERO(hwmon_dev);
}

static struct platform_driver dell_s3000_lpc_cpld_drv = {
        .probe      = dell_s3000_lpc_cpld_probe,
        .driver     = {
        .name       = "LPC",
        },
};
    
int dell_s3000_lpc_cpld_init(void)
{
        platform_device_register(&dell_s3000_lpc_cpld_dev);
        platform_driver_register(&dell_s3000_lpc_cpld_drv);

        return 0;
}

void dell_s3000_lpc_cpld_exit(void)
{
        platform_driver_unregister(&dell_s3000_lpc_cpld_drv);
        platform_device_unregister(&dell_s3000_lpc_cpld_dev);
}

MODULE_AUTHOR("Dhanakumar Subramanian  <dhanakumar_subramani@dell.com>");
MODULE_DESCRIPTION("Dell S3000 LPC CPLD driver");
MODULE_LICENSE("GPL");

module_init(dell_s3000_lpc_cpld_init);
module_exit(dell_s3000_lpc_cpld_exit);
