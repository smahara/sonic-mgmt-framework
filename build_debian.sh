...
if [[ $CONFIGURED_ARCH == armhf || $CONFIGURED_ARCH == arm64 ]]; then
    ...
    sudo http_proxy=$http_proxy debootstrap --variant=minbase --arch $CONFIGURED_ARCH stretch $FILESYSTEM_ROOT http://artifactory.force10networks.com/debian
fi
...
sudo LANG=C chroot $FILESYSTEM_ROOT add-apt-repository \
                                    "deb [arch=$CONFIGURED_ARCH] https://download.docker.com/linux/debian stretch test"
...
set /files/etc/sysctl.conf/net.ipv4.igmp_max_memberships 4096

set /files/etc/sysctl.conf/net.ipv4.neigh.default.gc_thresh1 1
set /files/etc/sysctl.conf/net.ipv6.neigh.default.gc_thresh1 1
