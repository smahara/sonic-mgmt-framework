...
DEB_BUILD_OPTIONS_GENERIC += autodbgsym 
...
define DOCKERIGNORES
# Auto-generated by slave.mk
**/dpkg_lock
*.log
endef
export DOCKERIGNORES
...
$(addprefix $(TARGET_PATH)/, $(DOCKER_IMAGES)) : $(TARGET_PATH)/%.gz : .platform docker-start \
	...
	if [ -z '$($*.gz_CACHE_LOADED)' ] ; then
		...
		# Create list of files to ignore within context sent to Docker server
		if [ ! -f $($*.gz_PATH)/.dockerignore ]; then echo "$$DOCKERIGNORES" >$($*.gz_PATH)/.dockerignore; fi
